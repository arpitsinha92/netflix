  ...
  return (
    <div className="relative min-h-screen">
        ...
        <div className="relative z-10 flex min-h-screen items-center justify-center px-4">
          <form className='w-full max-w-md bg-black/80 rounded-md p-8 text-white space-y-6'>
            <h1 className='font-bold text-3xl text-center'>{isSignInForm ? 'Sign In' : 'Sign Up'}</h1>
            <div className='space-y-4'>
              {!isSignInForm && (<input type="text" placeholder='Full Name' className='w-full rounded-md border border-white/10 bg-gray-800 p-3 text-sm outline-none' />)}
              <input type="text" placeholder='Email Address' className='w-full rounded-md border border-white/10 bg-gray-800 p-3 text-sm outline-none' />
              <input type="password" placeholder='Password' className='w-full rounded-md border border-white/10 bg-gray-800 p-3 text-sm outline-none' />
              <button className='w-full rounded-md bg-red-600 px-4 py-3 text-base font-semibold hover:bg-red-700'>{isSignInForm ? 'Sign In' : 'Sign Up'}</button>
            </div>
            <p className='text-center text-sm text-white/80 cursor-pointer' onClick={toggleIsSignInForm}>
